...
|Third-party Independent Software Vendors (ISV) |Power Platform connectors can’t be used with on-premises gateway connections at this time. |There are no plans to include this in the future roadmap. |If needed, consider moving your key content to a cloud platform like SharePoint or ServiceNow. |
...
|Third-party Independent Software Vendors (ISV) |The ESS Agent currently provides a limited set of prebuilt templates for both read and write scenarios. Additional read/write use cases can be implemented through custom configuration in Copilot Studio, which may involve creating custom topics and integrating with APIs - requiring technical expertise | By design – there are no current plans to expand this in the product roadmap | Collaborate with your Copilot Studio maker and technical teams to configure and extend the agent to support additional read/write scenarios through custom development |
|Third-Party Identity Providers & Single Sign-on |Third-party identity providers (IdPs) and third-party single sign-on (SSO) aren't currently supported. |We're currently evaluating the feasibility of including this in a future roadmap. |None currently. |
...
|Publishing | ESS Agent currently doesn’t support solution management capability in Power Platform as there are known issues in the platform to save customizations to a preferred solution and importing/exporting it to another Power Platform environment for ALM. | Working with Power Platform product group to address this issue and hoping to have this fixed by late summer 2025. | Manually import/export customizations from the default solution to the preferred solution and keep track of customizations documented in form of release notes. Currently, the app source package for ESS Agent is set to manual updates, so ensure that all customizations are saved in preferred solution before applying the update and import customizations back post-update.  |
...
|Licensing and payment | End users will be still able to continue consuming ESS Agent when there’s no Copilot license assigned to them and the billing plan is disabled, or the message capacity has been exhausted. | End users will not be able to consume ESS Agent when the billing usage enforcement is rolled out by late summer 2025. | Enforce a billing plan in Copilot Studio per environment where the ESS Agent is being published. |
...
| Security, Compliance and Privacy | Workers council region deployments not supported. Deployment to users in regions requiring workers council approval. (Such as France and Germany) | We're currently evaluating the feasibility of including this in a future roadmap.  | Focus preview deployments on users located outside of workers council countries where additional approvals are required for new tools like Employee Self-Service Agent. |
