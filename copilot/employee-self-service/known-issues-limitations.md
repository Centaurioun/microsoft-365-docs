...
|Security, compliance, and privacy | Workers council region deployments aren't supported. Deployment to users in regions requiring workers council approval (xuch as France and Germany). | We're currently evaluating the feasibility of including this functionality in a future roadmap. | Focus preview deployments on users located outside of workers council countries where additional approvals are required for new tools like Employee Self-Service Agent. |
