...
link:use-dkim-to-validate-outbound-email.md#DefaultDKIMbehavior[Default
...
steps to link:set-up-spf-in-office-365-to-help-prevent-spoofing.md[Set
up SPF to help prevent spoofing].
...
apply to sub.domain.contonos.com unless a different policy is explicitly
...
(EOP) marks the message as spoof instead of rejecting it. In other
words, for inbound email, Microsoft 365 treats `p=reject` and
`p=quarantine` the same way. Admins can define the action to take on
messages classified as spoof within the
link:set-up-anti-phishing-policies.md[anti-phishing policy].
...
* Admins can use the link:learn-about-spoof-intelligence.md[spoof
intelligence insight] or the
link:manage-tenant-allow-block-list.md[Tenant Allow/Block List] to allow
...
* link:anti-spam-message-headers.md[Anti-spam message headers] includes
...
link:how-office-365-uses-spf-to-prevent-spoofing.md[How Microsoft 365
uses Sender Policy Framework (SPF) to prevent spoofing]

link:set-up-spf-in-office-365-to-help-prevent-spoofing.md[*Set up SPF in
Microsoft 365 to help prevent spoofing*]
