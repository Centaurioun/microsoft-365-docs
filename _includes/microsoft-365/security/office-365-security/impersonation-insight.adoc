...
link:anti-spoofing-protection.md[domain spoofing], because the
impersonated domain is typically a real, registered domain. Messages
from senders in the impersonated domain can and often do pass regular
email authentication checks that would otherwise identify spoofing
attempts (SPF, DKIM, and DMARC).
