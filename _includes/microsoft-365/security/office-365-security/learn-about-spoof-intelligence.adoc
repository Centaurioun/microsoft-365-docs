...
information, see link:anti-spoofing-protection.md[Anti-spoofing
protection in EOP].
...
more information, see link:manage-tenant-allow-block-list.md[Manage the
...
link:recommended-settings-for-eop-and-office365-atp.md#eop-anti-phishing-policy-settings[EOP
...
link:how-office-365-uses-spf-to-prevent-spoofing.md[SPF],
link:use-dkim-to-validate-outbound-email.md[DKIM], and
link:use-dmarc-to-validate-email.md[DMARC]. However, the domain passed
our implicit email authentication checks
...
* A link to open link:threat-explorer.md[Threat Explorer] to see
...
information, see link:manage-tenant-allow-block-list.md[Use PowerShell
to manage spoofed sender entries to the Tenant Allow/Block List].
...
link:set-up-spf-in-office-365-to-help-prevent-spoofing.md[Set up SPF in
Microsoft 365 to help prevent spoofing]. For a more in-depth
understanding of how Office 365 uses SPF, or for troubleshooting or
non-standard deployments such as hybrid deployments, start with
link:how-office-365-uses-spf-to-prevent-spoofing.md[How Office 365 uses
...
link:use-dmarc-to-validate-email.md[Use DMARC to validate email in
Office 365].
