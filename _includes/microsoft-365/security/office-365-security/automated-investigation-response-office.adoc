...
link:office-365-air.md[Automatically investigate and respond to
threats].
...
uses the link:enable-the-report-message-add-in.md[Report Message add-in]
or the link:enable-the-report-phish-add-in.md[Report Phishing add-in] to
send it to Microsoft for analysis. The submission is also sent to your
system and is visible in Explorer in the *Submissions* view (formerly
referred to as the *User-reported* view). In addition, the user-reported
message now triggers a system-based informational alert, which
automatically launches the investigation playbook.
...
(link:exchange-online-protection-overview.md[EOP] and
(link:defender-for-office-365.md[Microsoft Defender for Office 365] to
see if there are any other similar messages reported by users.
...
investigation from a view in link:threat-explorer.md[Threat Explorer].
This investigation also creates an alert, so Microsoft 365 Defender
incidents and external SIEM tools can see that this investigation was
triggered.
...
* link:office-365-air.md[Get started using AIR]
